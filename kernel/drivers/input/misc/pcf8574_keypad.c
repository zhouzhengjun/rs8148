...
static const unsigned char ruisu_kp_btncode[] = {
	[0] = KEY_RESERVED,
	[1] = KEY_RIGHT,
	[2] = KEY_DOWN,
	[3] = KEY_UP,
...
static short read_byte(struct kp_data *lp)
{
	i2c_smbus_write_byte(lp->client, 0xff);
	return i2c_smbus_read_byte(lp->client);
...
static short read_state(struct kp_data *lp)
{
	unsigned char x, a;
	i2c_smbus_write_byte(lp->client, 0xff);
	x =  (~i2c_smbus_read_byte(lp->client)) & 0x3f;
	for (a = 0; x > 0; a++)
		x = x >> 1;
	return a;
}
...
static irqreturn_t pcf8574_kp_irq_handler(int irq, void *dev_id)
{
	...
	printk("<0>""the num of function called is %d\n", i++);
	printk("<0>""the key value is 0x%x\n", read_byte(lp));
...
static irqreturn_t pcf8574_kp_irq_handler(int irq, void *dev_id)
{
	...
	//printk("<0>""the num of function called is %d\n", i++);
	//printk("<0>""the key value is 0x%x\n", read_byte(lp));
	printk("<0>""the index of array ruisu_kp_btncode:%d\n", nextstate);
	
	/* 松开 : 最后一个参数: 0-松开, 1-按下 */
	if (lp->laststate != nextstate) {
		int key_down = nextstate > 0;
		unsigned short keycode = key_down ?
			lp->btncode[nextstate] : lp->btncode[lp->laststate];
