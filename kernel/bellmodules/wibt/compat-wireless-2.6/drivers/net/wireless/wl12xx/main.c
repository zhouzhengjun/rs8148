...
static int wl1271_set_rx_streaming(struct wl1271 *wl, bool enable)
{
	...
	if (enable) {
		wl->conf.rx_streaming.queues = 0x01;
...
static void wl1271_read_fw_crashlog(struct wl1271 *wl)
{
	int ret;
	int addr;
	u8 *ptr;
	...
	wl1271_info("Reading FW crash log");
	...
	ret = wl1271_ps_elp_wakeup(wl);
	if (!ret)
		wl1271_cmd_stop_fw_logger(wl);

	/* Read the first memory block address we should read, and mask out the
	 * FW's memory base */
	wl1271_fw_status(wl, wl->fw_status);
	addr = __le32_to_cpu(wl->fw_status->sta.log_start_addr) << 5;

	ptr = wl->fw_crashlog;
	for (i = 0; i < mem->req_logger_blocks; i++) {
		memset(ptr, 0, WL12XX_HW_BLOCK_SIZE);
		wl1271_read(wl, addr, ptr, WL12XX_HW_BLOCK_SIZE, false);

		/* The first 4 bytes of every memory block includes the
		 * hardware address of the next one divided by 32 */
		addr = __le32_to_cpup((__le32 *)ptr) << 5;
		if (!addr)
			break;
		ptr += WL12XX_HW_BLOCK_SIZE;
	}

	wl->fw_crashlog_size = i * WL12XX_HW_BLOCK_SIZE;
}
...
static void wl1271_recovery_work(struct work_struct *work)
{
	...
	wl->recovery_in_progress = true;

	if (wl->bss_type != BSS_TYPE_AP_BSS) {
		/* Read FW log */
		wl1271_read_fw_crashlog(wl);

		wake_up_interruptible(&wl->recovery_crashlog_q);
	}
...
static int wl1271_configure_suspend(struct wl1271 *wl)
{
	...
	if (!test_bit(WL1271_FLAG_PSM, &wl->flags)) {
		...
		wl1271_info("Done waiting for ps completion!");
			    ...
			    struct cfg80211_wowlan *wow)
{
	...
	if (wl->wow_enabled) {
		...
		/*
		 * we disable and then re-enable interrupts in order to flush
		 * the threaded_irq
		 */
		wl1271_disable_interrupts(wl);
			     ...
			     struct ieee80211_key_conf *key_conf)
{
	...
	case WL1271_CIPHER_SUITE_GEM:
		...
		if (wl->bss_type != BSS_TYPE_AP_BSS && cmd == SET_KEY)
			wl->quirks |= WL12XX_QUIRK_USE_2_SPARE_BLOCKS;
		tx_seq_32 = WL1271_TX_SECURITY_HI32(wl->tx_security_seq);
		tx_seq_16 = WL1271_TX_SECURITY_LO16(wl->tx_security_seq);
		break;
	default:
					...
					u32 changed)
{
	...
	if ((changed & BSS_CHANGED_ASSOC)) {
		if (bss_conf->assoc) {
			...
			/* resets the 2 spare blocks quirk when disassociating */
			wl->quirks &= ~WL12XX_QUIRK_USE_2_SPARE_BLOCKS;
			wl->quirks |= wl1271_get_fw_ver_quirks(wl);
			wl->aid = 0;
	...
	if (changed & BSS_CHANGED_IBSS) {
		...
		if (bss_conf->ibss_joined) {
			/* by default, use the default AP enabled rates */
			wl->rate_set = CONF_TX_AP_ENABLED_RATES;
			ret = wl1271_acx_sta_rate_policies(wl);
			if (ret < 0)
				goto out;
		}
...
static ssize_t wl1271_sysfs_show_fw_crashlog(struct device *dev,
					       struct device_attribute *attr,
					       char *buf)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
	...
	if (wl->bss_type == BSS_TYPE_AP_BSS) {
		mutex_unlock(&wl->mutex);
		len = snprintf(buf, PAGE_SIZE, "Not supported in AP mode\n");
		return len;
	}

	/* Let only one thread read the crashlog at a time, blocking others */
	while (wl->fw_crashlog_size == 0) {
		DEFINE_WAIT(wait);

		prepare_to_wait_exclusive(&wl->recovery_crashlog_q,
					  &wait,
					  TASK_INTERRUPTIBLE);

		if (wl->fw_crashlog_size != 0) {
			finish_wait(&wl->recovery_crashlog_q, &wait);
			break;
		}
	...
	/* Check if the crashlog is still valid */
	if (wl->fw_crashlog_size <= 0) {
		mutex_unlock(&wl->mutex);
		return -ENODEV;
	}

	/* Convert the FW crash log to "human readable" form */
	space_left = PAGE_SIZE - 1;
	len = wl->fw_crashlog_size;
	ptr = wl->fw_crashlog;
	wl->fw_crashlog_size = 0;

	while ((space_left > 0) && (len > 0)) {
		hex_dump_to_buffer(ptr, len, 16, 1, buf, space_left, false);
		processed_bytes = min(len, 16);
		buf += processed_bytes * 3;
		*(buf++) = '\n';
		space_left -= processed_bytes * 3 + 1;
		ptr += processed_bytes;
		len -= processed_bytes;
	}
	*buf = 0; /* Put a null terminator */

	mutex_unlock(&wl->mutex);

	return PAGE_SIZE - space_left;
}

static DEVICE_ATTR(fw_crashlog, S_IRUSR, wl1271_sysfs_show_fw_crashlog, NULL);
...
struct ieee80211_hw *wl1271_alloc_hw(void)
{
	...
	wl->tx_next_queue = 0;
        wl->recovery_in_progress = false;
	wl->fw_crashlog_size = 0;
	init_waitqueue_head(&wl->recovery_crashlog_q);
	...
	/* Allocate one page for the FW crash log */
	wl->fw_crashlog = (u8 *)get_zeroed_page(GFP_KERNEL);
	if (!wl->fw_crashlog) {
		ret = -ENOMEM;
		goto err_aggr;
	}
	...
	/* Create sysfs file for the FW crash log */
	ret = device_create_file(&wl->plat_dev->dev, &dev_attr_fw_crashlog);
	if (ret < 0) {
		wl1271_error("failed to create sysfs file fw_crashlog");
		goto err_hw_pg_ver;
	}
...
err_fw_crashlog:
	free_page((unsigned long)wl->fw_crashlog);
...
{
	/* Unblock any crashlog readers */
	mutex_lock(&wl->mutex);
	wl->fw_crashlog_size = -1;
	wake_up_interruptible_all(&wl->recovery_crashlog_q);
	mutex_unlock(&wl->mutex);

	device_remove_file(&wl->plat_dev->dev, &dev_attr_fw_crashlog);
	device_remove_file(&wl->plat_dev->dev, &dev_attr_hw_pg_ver);
	device_remove_file(&wl->plat_dev->dev, &dev_attr_bt_coex_state);
	platform_device_unregister(wl->plat_dev);
