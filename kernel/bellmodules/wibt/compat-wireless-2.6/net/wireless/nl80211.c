...
nl80211_wowlan_policy[NUM_NL80211_WOWLAN_TRIG] = {
	...
	[NL80211_WOWLAN_TRIG_PKT_PATTERN] = { .type = NLA_BINARY },
};
	...
	if (rdev->wowlan) {
		...
		if (rdev->wowlan->n_patterns)
			NLA_PUT(msg, NL80211_WOWLAN_TRIG_PKT_PATTERN,
				...
				sizeof(struct nl80211_wowlan_trig_pkt_pattern),
				rdev->wowlan->patterns);
	}
...
static int nl80211_set_wowlan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]) {
		int n_patterns;
		if (nla_len(tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]) %
				sizeof(struct nl80211_wowlan_trig_pkt_pattern))
			return -EINVAL;
		n_patterns = nla_len(tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]) /
				sizeof(struct nl80211_wowlan_trig_pkt_pattern);
		if (n_patterns > wowlan->n_patterns)
			return -EINVAL;
		new_triggers.patterns =
			kmemdup(nla_data(tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]),
				nla_len(tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]),
				GFP_KERNEL);
