...
{
	printk("<0>""mcasp_set_bits: reg:%x, val:%x", reg, val);
	__raw_writel(__raw_readl(reg) | val, reg);
}

static inline void mcasp_clr_bits(void __iomem *reg, u32 val)
{
	printk("<0>""mcasp_clr_bits: reg:%x, val:%x", reg, val);
	__raw_writel((__raw_readl(reg) & ~(val)), reg);
}
					 ...
					 unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		...
		printk("<0>""davinci_mcasp_set_dai_fmt: SND_SOC_DAIFMT_CBM_CFM has been set\n");
		break;
	...
	case SND_SOC_DAIFMT_NB_NF:
		...
		mcasp_clr_bits(base + DAVINCI_MCASP_TXFMCTL_REG, FSXPOL);
		#if 1
		mcasp_clr_bits(base + DAVINCI_MCASP_ACLKRCTL_REG, ACLKRPOL);//polarity revert
		#endif
		//mcasp_set_bits(base + DAVINCI_MCASP_ACLKRCTL_REG, ACLKRPOL);
		#if 0
		mcasp_clr_bits(base + DAVINCI_MCASP_RXFMCTL_REG, FSRPOL);
		#endif
		mcasp_set_bits(base + DAVINCI_MCASP_RXFMCTL_REG, FSRPOL); //polarity revert
		printk("<0>""davinci_mcasp_set_dai_fmt: SND_SOC_DAIFMT_NB_NF has been set\n");
		break;
					...
					struct snd_soc_dai *cpu_dai)
{
	...
	if (dev->version == MCASP_VERSION_2 && !fifo_level)
		...
		dma_params->acnt = dma_params->data_type; //pcm:word length
#if 0
	dma_params->acnt = 4;
#endif
...
static struct snd_soc_dai_driver davinci_mcasp_dai[] = {
	...
	#endif
//modify mark, for gs2971a properly.
	{
			.name		= "dm8148-mcasp",
...
static int davinci_mcasp_probe(struct platform_device *pdev)
{
	...
	printk("<0>""DM8148_mcasp.c: davinci_mcasp_probe is called !\n");
	...
	dev->clk = clk_get_sys("davinci-mcasp.0", NULL); //modify mark
	...
	printk("<0>""##### mem->start: %x, dev->base: %x#####\n", mem->start, dev->base);

	dev->op_mode = pdata->op_mode;
	dev->tdm_slots = pdata->tdm_slots;
	dev->num_serializer = pdata->num_serializer;
	dev->serial_dir = pdata->serial_dir;
	dev->codec_fmt = pdata->codec_fmt;// no use, default format no delay.
