...
#if defined(CONFIG_SND_DAVINCI_SOC_OSLEC)
#include "../../drivers/staging/echo/oslec.h"
#endif
...
static inline snd_pcm_uframes_t snd_pcm_capture_avail(struct snd_pcm_runtime *runtime)
{
	...
	//printk("%s():%d-%d=%d, bondary=%d,am=%d\n",__FUNCTION__,runtime->status->hw_ptr*4, runtime->control->appl_ptr*4,avail, runtime->boundary*4, runtime->control->avail_min*4);
	if (avail < 0)
		avail += runtime->boundary;
	return avail;
}
