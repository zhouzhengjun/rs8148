...
#else
	__raw_writel(0x0,GMII_SEL);	//GMII mode

	//Marvell 88E1111 RESET: GP1[7] > 10ms low
	//GP1[7] MUX1 -> PHY_RESET
	//regVal = PAD062_CNTRL;
	//PAD062_CNTRL = (volatile unsigned int) (BIT(19) | BIT(18) | BIT(17) |BIT(7));
...
#define PAD062_CNTRL  (*(volatile unsigned int *)(PADCTRL_BASE + 0x08F4))
...
#define MII_M1111_INT_POLARITY_HIGH			0x0400			
...
static void phy_init(char *name, int addr)
{
...
#if 0
	val = miiphy_read(name,addr, MII_M1111_PHY_EXT_SR, &val);
	val &= ~MII_M1111_HWCFG_FIBER_COPPER_AUTO;
	val &= ~MII_M1111_INT_POLARITY_HIGH;
	miiphy_write(name,addr, MII_M1111_PHY_EXT_SR, val);
#endif	
	...
	printf("AutoNegotiate check count = %d\n",cntr);	
...
#define MII_M1111_PHY_LED_MANUAL	0x19
#define LED_DUPLEX_SHIFT		10
#define LED_LINK10_SHIFT		8
#define LED_LINK100_SHIFT	6
#define LED_LINK1000_SHIFT	4
#define LED_RX_SHIFT			2
#define LED_TX_SHIFT			0
#define LED_MODE_MASK	0x0000FFF
#define LED_MODE_NORMAL	0
#define LED_MODE_BLINK	1
#define LED_MODE_OFF		2
#define LED_MODE_ON		3

#define MII_M1111_PHY_EPSCR	20
#define EPSCR_LINELOOP		0x00004000 

static int do_marvell_phy_test(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	unsigned short val;
	unsigned char mode=LED_MODE_NORMAL;
...
//	phy_init("cpsw",0x18);
	...
	val = miiphy_read("cpsw",0x18, MII_M1111_PHY_LED_CONTROL, &val);

	val = miiphy_read("cpsw",0x18, MII_M1111_PHY_LED_MANUAL, &val);
	val &= ~LED_MODE_MASK;
	val |=(mode<<LED_DUPLEX_SHIFT)|
		(mode<<LED_LINK10_SHIFT)|
		(mode<<LED_LINK100_SHIFT)|
		(mode<<LED_LINK1000_SHIFT)|
		(mode<<LED_RX_SHIFT)|
		(mode<<LED_TX_SHIFT);

	miiphy_write("cpsw",0x18, MII_M1111_PHY_LED_MANUAL, val);

	miiphy_read("cpsw",0x18, MII_M1111_PHY_EPSCR, &val);
	val |= EPSCR_LINELOOP;
	miiphy_write("cpsw",0x18, MII_M1111_PHY_EPSCR, val);
	miiphy_read("cpsw",0x18, MII_M1111_PHY_EPSCR, &val);
...
U_BOOT_CMD(
	phy, 2, 1,	do_marvell_phy_test,
	"Marvell 88E1111 phy test",
	""
);
