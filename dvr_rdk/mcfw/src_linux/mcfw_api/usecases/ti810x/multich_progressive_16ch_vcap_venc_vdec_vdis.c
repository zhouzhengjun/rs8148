            ...
            )
{
    ...
    */
    ...
    {
        selectPrm->outQueChInfo[1].inChNum[chId] = chId+1;
    }
...
static void selectCaptureFullFpsChs(UInt32 numChs, UInt32 fullFpsChs[])
{
    ...
    prm.queId = 0;
    prm.skipAlternateFramesChBitMask = 0xFFFF;

    for(chId=0; chId<numChs; chId++)
    {
        prm.skipAlternateFramesChBitMask
            &= ~(1<<fullFpsChs[chId]);
    }
...
Void MultiCh_createProgressive16ChVcapVencVdecVdis()
{
    ...
    setLinkIds(enableOsd, mergeId, dupId, ipcId, selectId);
    ...
    if (enableAlgLink)
    {
        mergePrm[D1_CIF_QCIF_MERGE_LINK_IDX].outQueParams.nextLink  = gVcapModuleContext.dspAlgId[0];

        ipcOutVpssPrm.inQueParams.prevLinkId                   = gVcapModuleContext.dspAlgId[0];
        ipcOutVpssPrm.inQueParams.prevLinkQueId                = ALG_LINK_FRAMES_OUT_QUE;

        dspAlgPrm.inQueParams.prevLinkId                         = mergeId[D1_CIF_QCIF_MERGE_LINK_IDX];
        dspAlgPrm.inQueParams.prevLinkQueId                      = 0;
        dspAlgPrm.outQueParams[ALG_LINK_FRAMES_OUT_QUE].nextLink = ipcId[IPC_OUT_VPSS_LINK_IDX];
    }
    else
    {
