...
Int32 Acap_init(ACAP_PARAMS_S * pContext)
{
    if (pContext)
    {
        ...
        Audio_startCapture(&gAcapModuleContext.acapParams);
    }
    return ERROR_NONE;
}
...
{
    if (pPrm)
        return Audio_getCapChData(chNum, pPrm);
    else
        return ERROR_FAIL;
}
    ...
    if (pPrm)
    {
        switch (pPrm->encoderType)
        {
            case AUDIO_CODEC_TYPE_AAC_LC:
            {
                ...
                return Audio_createAacEncAlgorithm(handle, pPrm);
            }
            break;
            ...
            {
                handle = Audio_allocMem(Audio_getEncoderContextSize(pPrm->encoderType));
                return Audio_createG711EncAlgorithm(handle);
            }
        }
    }
    return handle;
    ...
    if (pPrm)
    {
        switch (pPrm->decoderType)
        {
            case AUDIO_CODEC_TYPE_AAC_LC:
            {
                ...
                return Audio_createAacDecAlgorithm(handle, pPrm);
            }
            break;
            ...
            {
                handle = Audio_allocMem(Audio_getDecoderContextSize(pPrm->decoderType));
                return Audio_createG711DecAlgorithm(handle);
            }
        }
    }
    return handle;
