...
//ruisutech add
//the input buffer image is YUYV format(Y,U,Y,V, interlaced in only one planar)
// output 420p, (Y,U,V in three planar)
...
typedef struct {
  ...
  int imageWidth;   // in bytes
  int imageHeight;  // in lines
  
  int srcOffsetH;  // in bytes
  int dstOffsetH;  // in bytes
    
} OSA_DmaYUYVtoYUV420p;
...
int OSA_dmaCopyYUYVtoYUV420p(OSA_DmaChHndl *hndl, OSA_DmaYUYVtoYUV420p *prm, int count );
