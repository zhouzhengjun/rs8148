...
Int32 Utils_dmaCopyUV(Utils_DmaChObj *pObj, Utils_DmaCopyUV *pInfo, UInt32 numTransfers)
{
    EDMA3_DRV_Result edma3Result = EDMA3_DRV_SOK;
    EDMA3_DRV_PaRAMRegs *pParamSet;
    UInt32 i;

    /* UV will be transferred separately */
    if(numTransfers>pObj->maxTransfers/2) return FVID2_EFAIL;
    ...
    //U
        ...
        pParamSet->srcBIdx    = 2;
        pParamSet->destBIdx   = 1;
        pParamSet->srcCIdx    = pInfo->width*2;
        pParamSet->destCIdx   = pInfo->width;

        pParamSet->aCnt       = 1;
        pParamSet->bCnt       = pInfo->width;
        pParamSet->cCnt       = pInfo->height;
	...
	//V
        ...
        pParamSet->destAddr   = (UInt32)pInfo->destAddr+pInfo->width*pInfo->height;
